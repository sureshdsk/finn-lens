    ...
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Total Invested", value: fmt(totalPurchase), sub: "purchase cost basis", accent: "text-amber-600 dark:text-amber-400" },
      ...
      <div className="grid lg:grid-cols-3 gap-5">
                ...
                return (
                  <div key={type} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-foreground">{cfg?.label || type}</span>
                    </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="relative z-10">
            <h3 className="text-base font-semibold text-foreground mb-4">Performance Ranking</h3>
                  ...
                  <motion.div key={a.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 + i * 0.04 }}
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{cfg.label} • {a.purchaseDate.slice(0, 4)}</div>
                      </div>
                    </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button onClick={openAdd} className="bg-primary text-primary-foreground font-medium shadow-sm hover:bg-primary/90 rounded-md text-xs flex items-center gap-1.5 px-3 py-1.5">
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="relative z-10 space-y-3">
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <input value={fName} onChange={(e) => setFName(e.target.value)} placeholder="Farmland - Nashik"
                    className="w-full h-8 rounded-md bg-card border border-border shadow-sm px-2 text-xs text-foreground bg-transparent focus:outline-none" />
                </div>
                <div>
              ...
              <button onClick={save} className="bg-primary text-primary-foreground font-medium shadow-sm hover:bg-primary/90 rounded-md text-xs flex items-center gap-1.5 px-3 py-1.5">
        ...
        {filtered.map((asset, i) => {
          ...
          return (
            <motion.div key={asset.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }}
              className="bg-card border border-border shadow-sm rounded-lg overflow-hidden group">
              <div className="relative z-10">
                <button onClick={() => setExpandedId(isExpanded ? null : asset.id)}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      <div className="w-9 h-9 rounded-lg bg-card border border-border shadow-sm flex items-center justify-center shrink-0"
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs px-1.5 py-0.5 rounded-md bg-card border border-border shadow-sm" style={{ color: cfg.color }}>{cfg.label}</span>
                        </div>
                        {asset.location && (
                      ...
                      <div className="p-4 space-y-3">
                        <div className="grid grid-cols-3 gap-2">
                          {[
                            ...
                            <div key={m.label} className="bg-muted/50 rounded-lg p-2 border border-border">
                              <div className="text-xs text-muted-foreground mb-0.5">{m.label}</div>
                              <div className={`text-xs font-semibold ${m.accent}`}>{m.value}</div>
                            </div>
                          ))}
                        </div>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {asset.documents.map((d) => (
                                <span key={d} className="text-xs px-1.5 py-0.5 rounded-md bg-secondary text-muted-foreground">{d}</span>
                              ))}
                            </div>
                          </div>
                        )}
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => openEdit(asset)}
                            className="text-xs px-2 py-1 rounded-md text-muted-foreground hover:text-primary hover:bg-secondary/50 transition-all flex items-center gap-1">
                            <Pencil className="w-2.5 h-2.5" /> Edit
                          </button>
                          <button onClick={() => remove(asset.id)}
                            className="text-xs px-2 py-1 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/5 transition-all flex items-center gap-1">
