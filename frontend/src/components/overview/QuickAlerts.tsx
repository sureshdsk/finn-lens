  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}
      ...
      <div className="relative z-10">
        <h3 className="font-semibold text-xs uppercase tracking-wider text-foreground mb-4">Alerts</h3>
          ...
          {quickAlerts.slice(0, 4).map((a, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                ...
                className="flex items-start gap-2.5 bg-card border border-border shadow-sm rounded-sm p-2.5">
